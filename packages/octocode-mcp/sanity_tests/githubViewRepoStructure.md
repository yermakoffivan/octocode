# Sanity check — `githubViewRepoStructure`

> List a repo directory tree.
>
> **Manual runtime checklist** — run each step against the live MCP tool (or `benchmark/github/scripts/call-tool.mjs githubViewRepoStructure '<queries-json>'`) and tick the box.
...
- [ ] Entries paginate via `entryPageNumber`/`entriesPerPage`; large trees report `truncated`/`hasMore`.
...
## Example call
```json
{ "queries": [ {"owner":"bgauryy","repo":"octocode-mcp","path":"","depth":1} ], "responseCharLength": 500 }
```
