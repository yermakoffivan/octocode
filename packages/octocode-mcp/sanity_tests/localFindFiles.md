# Sanity check — `localFindFiles`

> Find files by name/glob/size/churn.
>
> **Manual runtime checklist** — run each step against the live MCP tool (or `benchmark/github/scripts/call-tool.mjs localFindFiles '<queries-json>'`) and tick the box.
...
- [ ] Pagination knob(s) accepted: `filePageNumber`, `filesPerPage`.
...
- [ ] Per-query knob (`filePageNumber` / `filesPerPage`) returns the next page on increment.
- [ ] (no tool-specific pagination notes)
...
- [ ] The size knob (`filePageNumber` / `filesPerPage`) lets you fetch exactly what you need — the tool never over-returns.

## Known gaps
- ⚠️ `maxFiles` (= `limit` ‖ 1000) caps discovery BEFORE pagination; beyond it set a wider `limit`/narrower glob (sets `wasFileCapped`) — pagination issue #3.

## Example call
```json
{ "queries": [ {"path":".","filesPerPage":20,"filePageNumber":1} ], "responseCharLength": 500 }
```
