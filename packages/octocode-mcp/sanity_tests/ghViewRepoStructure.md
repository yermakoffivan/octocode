# Sanity check — `ghViewRepoStructure`

> List a repo directory tree.
>
> **Manual runtime checklist** — run each step against the live MCP tool (or `benchmark/github/scripts/call-tool.mjs ghViewRepoStructure '<queries-json>'`) and tick the box.
...
- [ ] Entries paginate via `page`/`itemsPerPage`; large trees report `truncated`/`hasMore`.
