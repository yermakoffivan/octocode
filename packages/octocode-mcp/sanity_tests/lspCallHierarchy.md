# Sanity check — `lspCallHierarchy`

> Incoming callers / outgoing callees of a function.
>
> **Manual runtime checklist** — run each step against the live MCP tool (or `benchmark/github/scripts/call-tool.mjs lspCallHierarchy '<queries-json>'`) and tick the box.
...
- [ ] Pagination knob(s) accepted: `callsPerPage`, `page`.
...
- [ ] Per-query knob (`callsPerPage` / `page`) returns the next page on increment.
- [ ] Calls paginate via `callsPerPage`/`page`; `depth`/`contextLines` control fan-out + snippet size.
...
- [ ] The size knob (`callsPerPage` / `page`) lets you fetch exactly what you need — the tool never over-returns.

## Known gaps
- ⚠️ Each node's `content` snippet is clipped to 500 chars (unsplittable-node backstop) rather than char-paged — pagination issue #4.

## Example call
```json
{ "queries": [ {"uri":"file:///abs/path.ts","symbolName":"foo","lineHint":10,"direction":"incoming","callsPerPage":15,"page":1} ], "responseCharLength": 500 }
```
