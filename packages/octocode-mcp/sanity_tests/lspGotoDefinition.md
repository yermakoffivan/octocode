# Sanity check — `lspGotoDefinition`

> Resolve a symbol to its definition.
>
> **Manual runtime checklist** — run each step against the live MCP tool (or `benchmark/github/scripts/call-tool.mjs lspGotoDefinition '<queries-json>'`) and tick the box.
...
- [ ] Needs a `lineHint` from a prior search.
...
## Example call
```json
{ "queries": [ {"uri":"file:///abs/path.ts","symbolName":"foo","lineHint":10} ], "responseCharLength": 500 }
```
