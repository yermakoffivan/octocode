# Sanity check — `githubSearchPullRequests`

> Search PRs or read one PR deeply.
>
> **Manual runtime checklist** — run each step against the live MCP tool (or `benchmark/github/scripts/call-tool.mjs githubSearchPullRequests '<queries-json>'`) and tick the box.
...
- [ ] Pagination knob(s) accepted: `charOffset`, `charLength`, `page`.
...
- [ ] Per-query knob (`charOffset` / `charLength` / `page`) returns the next page on increment.
- [ ] Comments now page **all** GitHub pages (no 100/30 cap); body + fileChanges returned in **full**, bounded only by the char cursor.
- [ ] `type=metadata|partialContent|fullContent` controls diff detail.
...
- [ ] The size knob (`charOffset` / `charLength` / `page`) lets you fetch exactly what you need — the tool never over-returns.
...
## Example call
```json
{ "queries": [ {"prNumber":27733,"owner":"facebook","repo":"react","withComments":true} ], "responseCharLength": 500 }
```
