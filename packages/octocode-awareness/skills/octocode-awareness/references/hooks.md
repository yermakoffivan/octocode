...
| verify gate | `Stop` / `SubagentStop` | `Stop` / `SubagentStop` | `stop` / `subagentStop` | `agent_end` | `scripts/hooks/stop-verify.sh` / `wirePiAwarenessHooks(pi)` | Shell hooks hard-block conclusion once (exit 2); Pi cannot hard-block after `agent_end`, so it injects one follow-up reminder for each unchanged pending-task set. | `octocode-awareness verify mark --agent-id <id> --workspace "$PWD" --all-pending --message "<check>" --compact`, then rerun `verify audit`. |
...
Behavior details:
- **agent id** = `OCTOCODE_AGENT_ID` if set, else an explicit hook payload `agent_id`/`agentId`, else the payload's `session_id`/`sessionId`; Pi falls back to `pi:<session-file-basename>` then `pi:<pid>`.
...
- **Pre/post correlation** — shell hooks store one small task-correlation file per hook event under the awareness home.
  If a host drops post-edit or the correlation file is missing, post-edit fails open and leaves the lock for TTL expiry plus verify/prune cleanup.
...
- **Bounded waits** — hooks never sleep indefinitely. A wrapper that chooses to wait should call `lock wait` or `lock acquire --wait-seconds`; both return `2` with `conflicts[]` on timeout, sleep outside SQLite transactions, and reject waits above 3600 seconds.
...
## Smart host install matrix

| Host | Install/check path | Script or adapter | Agent verification |
|---|---|---|---|
| Claude Code | `hooks install --host claude --project-dir .`; `hooks check --host claude --strict` | `${CLAUDE_PROJECT_DIR}/packages/octocode-awareness/dist/skills/octocode-awareness/scripts/hooks/*.sh` | Strict check must report no missing/drifted entries; frontmatter hooks still require the skill to be active unless installed project-wide. |
| Codex | `hooks install --host codex --project-dir .`; `hooks check --host codex --strict` | absolute `dist/skills/octocode-awareness/scripts/hooks/*.sh` in `.codex/hooks.json` | Strict check must pass; ensure host hooks are enabled for the session. |
| Cursor | `hooks install --host cursor --project-dir .`; `hooks check --host cursor --strict` | absolute `dist/skills/octocode-awareness/scripts/hooks/*.sh` in `.cursor/hooks.json` | Strict check must pass; Cursor cloud supports fewer event classes, so smoke a write path. |
| Pi | Do not run shell `hooks install`; use `wirePiAwarenessHooks(pi, { skillRoot })` or `@octocodeai/pi-extension`. | `createPiAwarenessBridge().handleToolCall/handleToolResult` plus `agent_end` verify reminder. | Smoke `tool_call` -> lock, `tool_result` -> pending verify, and `agent_end` -> one reminder per pending-task set. |
...
- `hooks check --strict` requires the exact generated command path, matcher, and timeout. It still recognizes older awareness hook roots as awareness-owned drift so install/remove can repair them.
...
`hooks install` writes config files; the host must still execute them.

| Host | Checklist |
|---|---|
| Codex | Project/user hooks.json present; Codex hooks feature/flag enabled for the session; matcher covers write tools. |
| Cursor | `.cursor/hooks.json` or user hooks loaded; third-party/Claude settings enabled if relying on those; verify with a deliberate write that should claim a lock. |
| Claude | Skill frontmatter hooks run only when the skill is loaded for the turn. |
| Pi | Uses in-process bridge (`wirePiAwarenessHooks`), not skill frontmatter. |

After install: `hooks check --host <host> --strict --compact`, then smoke `lock acquire` + an edit under the same `OCTOCODE_AGENT_ID`.
