...
| EXT-CODE-03 | `octocode search` | Does remote shorthand with `--lang ts` route to GitHub code search correctly? | `octocode search "_streamChatModelEvents" langchain-ai/langchainjs --lang ts --json --compact` | Non-empty TypeScript-scoped results or an honest indexing/auth diagnostic; no local-only language alias bug. |
...
| EXT-REMOTE-LOCAL-01 | `--repo` quick commands | Can remote-as-local preserve repo-relative paths? | `octocode search "createStore" src --repo pmndrs/zustand --lang ts --json --compact`, then `octocode search src/vanilla.ts --repo pmndrs/zustand --content-view exact --match-string createStore --json --compact`. | First call materializes or fetches; second uses the same repo/path identity and exact content anchor. |
...
| EXT-NPM-02 | `octocode search` | Does packages target distinguish exact package from keyword search? | `octocode search zustand --target packages --json --compact` | Exact package metadata returns source repo; keyword ambiguity is explicit if present. |
