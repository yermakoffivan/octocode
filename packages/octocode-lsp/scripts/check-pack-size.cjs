 ...
 * optional per-platform packages, so the root package must stay a JS/type
 * loader and must never include a `.node` artifact.
 */
'use strict'

const { spawnSync } = require('child_process')

const MAX_PACKED_BYTES = Number(process.env.OCTOCODE_LSP_PACK_MAX_BYTES ?? 2_000_000)
const MAX_UNPACKED_BYTES = Number(process.env.OCTOCODE_LSP_UNPACKED_MAX_BYTES ?? 6_000_000)
