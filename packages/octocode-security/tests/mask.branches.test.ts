...
describe('maskSensitiveData - edge cases for match array access', () => {
  ...
  it.skip('[rust: mock-based, N/A] should handle pattern array index bounds correctly', async () => {
    vi.doMock('../src/regexes/index.js', () => ({
      allRegexPatterns: [
        {
...
describe('maskSensitiveData - overlapping matches branch (lines 52-56)', () => {
  ...
  it.skip('[rust: mock-based, N/A] should skip overlapping matches and keep only the first one', async () => {
    vi.doMock('../src/regexes/index.js', () => ({
      allRegexPatterns: [
        {
  ...
  it.skip('[rust: mock-based, N/A] should trigger overlapping branch when inner pattern starts inside outer match', async () => {
    vi.doMock('../src/regexes/index.js', () => ({
      allRegexPatterns: [
        {
  ...
  it.skip('[rust: mock-based, N/A] should correctly handle adjacent non-overlapping matches', async () => {
    vi.doMock('../src/regexes/index.js', () => ({
      allRegexPatterns: [
        {
          regex: /TOKEN="[^"]{10,}"/gi,
...
describe('maskSensitiveData - nonOverlapping array iteration (lines 60-69)', () => {
  ...
  it.skip('[rust: mock-based, N/A] should correctly process multiple non-overlapping matches in reverse', async () => {
    vi.doMock('../src/regexes/index.js', () => ({
      allRegexPatterns: [
        {
          regex: /KEY\d="[^"]{10,}"/gi,
