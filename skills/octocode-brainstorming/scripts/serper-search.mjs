...
Environment: process env, then <workspace>/.octocode/.env, then <OCTOCODE_HOME>/.env`);
  ...
  if (opts.check) {
    if (!apiKey) {
      ...
      console.log(`Add SERPER_API_KEY to: ${envPath}`);
      process.exitCode = 1;
      return;
    }
    ...
    try {
      ...
      console.log(`Update SERPER_API_KEY in: ${envPath}`);
      process.exitCode = 1;
    }
    return;
  }
  ...
  if (!apiKey) {
    die(`SERPER_API_KEY is not set. Add it to ${envPath} or export it in your shell.`);
    return;
  }
  if (!opts.query) {
