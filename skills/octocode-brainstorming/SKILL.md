...
Evidence-grounded idea exploration: diverge first, validate local/GitHub/package/web evidence, stress-test claims, then decide. Flow: `FRAME -> DIVERGE -> RESEARCH -> CROSS-POLLINATE -> STRESS-TEST -> SYNTHESIZE -> DECIDE`.
...
Always declare a Surface Plan before searching: Local active/skipped, GitHub/packages active/skipped, Web active/skipped, each with a reason. Local goes first when the idea targets the user's repo.
...
- Cross-pollinate at least once per active surface: web names feed repo/package search, repo/package clues feed web and code reads.
- Keep a claim ledger: `claim -> source -> confidence -> next query`.
