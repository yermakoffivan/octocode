...
`skill` is the agent-safe skill installer. It never prompts, so automation can use `--name <octocode-skill>`, `--add <github-path>`, or `--install-all`, `--platform common|cursor|claude|codex|opencode|pi|copilot|gemini|all`, `--mode symlink|copy|hybrid`, `--force|--update`, and `--json` deterministically.
...
| [SKILLS_GUIDE.md](https://github.com/bgauryy/octocode/blob/main/docs/SKILLS_GUIDE.md) | Install GitHub Agent Skill folders, skill libraries, and Octocode skills into supported agent clients |
