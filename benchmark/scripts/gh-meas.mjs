#!/usr/bin/env node
// gh-meas.mjs — Run a bare `gh` command, log character counts and elapsed_ms to $LOG.
//
// Mirrors the same ruler used by rtk-meas.mjs:
...
//
// Used by the rtk-gh researcher when a bare `gh` call is needed instead of
// a `rtk gh` command (e.g. for raw API access without RTK output filtering).
//
...
if (!existsSync(SENTINEL)) {
  console.error(`gh-meas: ${SENTINEL} missing — run benchmark/scripts/set-q.sh <n> first`);
  process.exit(2);
}
